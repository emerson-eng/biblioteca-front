		...
		<q-dialog v-model="dialog" :maximized="$q.screen.width < 700 ? true : false" transition-show="slide-up" transition-hide="slide-down" @show="initUpdate">
			<q-card style="width: 650px; max-width: 650px" class="q-px-sm">
				<q-card-section class="row items-center q-pb-none">
					<div class="text-h6 text-color-dark text-bold">Registrar sección</div>
					<q-space />
...
const register = () => {
	...
	post('admin/section', form.value).then(response => {
		console.log('register', response)
		if(response.status >= 200 && response.status < 300) {
			let data = response.data.data
			data.state = 1
			let sections = [data, ...dataTablePinia.sections]
...
const update = () => {
	loadingBtn.value = true
	const url = `admin/section/${props.selectRow.id}`
