<template>
	<q-page padding>
		...
		<data-table
		exportName="Editoriales" :keyTable="'editorials'"
		:loading="loading"
		:columns="columns"
		:rows="dataTablePinia.editorials"
		:keyName="'id'"
		@editRow="editRow"
		@deleteRow="deleteRow"
		class="q-mt-md"
...
const getData = () => {
	loading.value = true
	get('admin/editorial', false).then((response) => {
		console.log('editorial', response)
		if(response.status >= 200 && response.status < 300) {
			formatStateModels(response.data.data)
			dataTablePinia.setEditorials(response.data.data)
		}
	}).finally(() => {
		loading.value = false
	})
}
if(dataTablePinia.editorials.length === 0) 
	getData()
else
	formatStateModels(dataTablePinia.editorials)
...
const changeSwith = (row) => {
	...
	const url = `admin/editorial/state/${row.id}`
...
const deleteRow = (row) => {
	...
	const url = `admin/editorial/${row.id}`
