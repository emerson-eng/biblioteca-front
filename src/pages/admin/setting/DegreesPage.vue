<template>
	<q-page padding>
		...
		<data-table
		exportName="Grados" :keyTable="'degrees'"
		:loading="loading"
		:columns="columns"
		:rows="dataTablePinia.degrees"
		:keyName="'id'"
		@editRow="editRow"
		@deleteRow="deleteRow"
		class="q-mt-md"
...
import CreateDegree from 'components/admin/dialogs/setting/CreateDegree.vue'
...
const getData = () => {
	loading.value = true
	get('admin/degree', false).then((response) => {
		console.log('degree', response)
		if(response.status >= 200 && response.status < 300) {
			formatStateModels(response.data.data)
			dataTablePinia.setDegrees(response.data.data)
		}
	}).finally(() => {
		loading.value = false
	})
}
if(dataTablePinia.degrees.length === 0) 
	getData()
else
	formatStateModels(dataTablePinia.degrees)
...
const changeSwith = (row) => {
	...
	const url = `admin/degree/state/${row.id}`
	const form = { state: row.state == 1 ? 0 : 1 }
	put(url, form).then((response) => {
		if(response.status >= 200 && response.status < 300) {
			row.state = response.data.data.state
			let degrees = dataTablePinia.degrees.map((item) => {
				if(item.id == row.id)
					item = row
				return item
			})
			dataTablePinia.setDegrees(degrees)
			loanPinia.setDegrees([])
		}
	}).finally(() => {
...
const deleteRow = (row) => {
	...
	const url = `admin/degree/${row.id}`
	deleteApi(url, notif).then((response) => {
		if(response.status >= 200 && response.status < 300) {
			let data = dataTablePinia.degrees.filter((item) => item.id != row.id)
			dataTablePinia.setDegrees(data)
			loanPinia.setDegrees([])
		}
	})
}
